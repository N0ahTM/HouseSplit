...
3. Open a person sheet to add one or more arrival and checkout ranges.
...
Exchange rates are loaded online from the Frankfurter API with the `ECB` provider. The request contains only the source and target currency, not the rent amount, names, or stay dates. The last successful rate is cached locally as a fallback if the device is later offline.
